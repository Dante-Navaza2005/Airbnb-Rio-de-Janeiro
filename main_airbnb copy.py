...
#? As now we are only intrested in the Extra Trees model, we will comment out the for loop iterating through all the models, and only focus on the Extra Trees model.
"""
...
#? After printing the dictionary containing the features and their importance in percentage, we will save said dictionary in a variable so we can comment out the previous code that trained the model (as we will train a final model with the data we will filter based on the most important features).
#? This will be useful in avoiding code repetition and reducing the amount of time taken to run the entire script.  
