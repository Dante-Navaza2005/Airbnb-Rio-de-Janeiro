...
#? We will create three dictionaries for each different data type (numerical, boolean and lists). We will then create a button for each dictionary and update the values of each column depending on the input.
...
# Print the value of final_model_file for debugging
st.write(f"Provided model file path: {final_model_file}")

#? Creating a button for each dictionary and updating the values after the user's input
#? Some buttons such as the latitude and longitude, will have float values and others will have integer, we will adjust their values as needed.
